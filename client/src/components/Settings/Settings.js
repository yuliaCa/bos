...
import ButtonSelect from "../../pages/RegistrationPage/ButtonSelect";
...
function SettingsPage(props) {
  ...
  function handleClick(event) {
    ...
    // Update user's email
    if (input.userEmailAddress !== undefined) {
      firebase
        .updateEmail(firebase.auth, input.userEmailAddress)
        .then(() => {
        ...
        .catch((error) => {
          ...
          console.error(
            `There was an error updating the email: ${errorCode}, ${errorMessage}`
          );
        });
    }
    ...
    if (input.fullname !== undefined) {
      firebase
        ...
        .then(() => {
          console.log(`Fullname updated: ${firebase.auth.currentUser.uid}`);
        })
        .catch((error) => {
    ...
    axios.patch(`/update/${uid}`, updateProfile).catch((error) => {
      if (error.response) {
        console.log(`There was an error updating profile: ${error.response.data}`);
      }
    });
  }
